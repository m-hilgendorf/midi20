...
use crate::message::flex_header::{FlexAddress, FlexStatus};
use crate::message::flex_payload::{FlexChordName, FlexKeySignature, FlexMetronome, FlexTimeSignature};
use crate::packet::*;
...
impl Flex {
    ...
    // pub fn text_message(&self) -> Self::Data {
    //     todo!()
    // }
...
pub mod flex_header {
    ...
    impl From<u8> for FlexAddress {
        fn from(value: u8) -> Self {
            ...
            match address {
                ...
                2 => Self::_Reserved1(destination),
                3 => Self::_Reserved2(destination),
                _ => unreachable!(),
            }
        }
    }
    ...
    impl From<FlexAddress> for u8 {
        fn from(value: FlexAddress) -> Self {
            match value {
                FlexAddress::Channel(destination) => 0 << 4 | destination,
                FlexAddress::Group(destination) => 1 << 4 | destination,
                FlexAddress::_Reserved1(reserved) => 2 << 4 | reserved,
                FlexAddress::_Reserved2(reserved) => 3 << 4 | reserved,
            }
        }
    }
    ...
    impl From<u16> for FlexStatus {
        fn from(value: u16) -> Self {
            ...
            match status_bank {
                0x00 => FlexStatus::SetupAndPerformance(status.into()),
                0x01 => FlexStatus::MetadataText(status.into()),
                0x02 => FlexStatus::PerformanceTextEvent(status.into()),
    ...
    impl From<FlexStatus> for u16 {
        fn from(value: FlexStatus) -> Self {
            match value {
                FlexStatus::SetupAndPerformance(status) => 0x00 << 8 | u8::from(status) as u16,
    ...
    pub enum FlexMetadataText {
        ...
        /// The data of the Recording Date message shall also conform to ISO 8601 for the Date with an optional time
        RecordingDate,
        RecordingLocation,
    }
    ...
    pub enum FlexPerformanceTextEvent {
        ...
        LyricsLanguage,
        /// Contains Ruby lyrics as Unicode UTF-8 text
        RubyLyrics,
        /// Contains a BCP 47 language identifier.
        RubyLyricsLanguage,
    }
    ...
    mod tests {
        mod header {
            use crate::message::flex_header::{FlexAddress, FlexMetadataText, FlexPerformanceTextEvent, FlexSetupAndPerformance, FlexStatus};
            ...
            fn status_from_u16() {
                ...
                assert_eq!(FlexStatus::MetadataText(0.into()), u16::from_le(0x0100_u16).into());
                assert_eq!(
                    FlexStatus::PerformanceTextEvent(0.into()),
                    u16::from_le(0x0200_u16).into()
                );
                assert_eq!(FlexStatus::Reserved(u16::from_le(0x03_00)), u16::from_le(0x0300_u16).into());
            }
    ...
    mod tests {
        ...
        fn metronome_into_u32_array() {
            let a: [u32; 2] = FlexMetronome {
                ...
                .into();
            assert_eq!([u32::from_le(0xD4C3_B2A1_u32), u32::from_le(0x0000_F6E5)], a);
        ...
        fn chord_name_from_u32_3() {
            ...
            let source = [u32::from_le(0xD4C3_B2A1_u32), u32::from_le(0x_0000_F6E5), u32::from_le(0x3C2B_1AF0)];
